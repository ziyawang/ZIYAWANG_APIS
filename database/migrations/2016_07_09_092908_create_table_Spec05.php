...
class CreateTableSpec05 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('T_P_SPEC05', function (Blueprint $table) {
            $table->increments('Spec05ID')->comment('典当担保自增主键');
            $table->decimal('TotalMoney')->comment('金额');
            $table->string('AssetType',64)->comment('类型');
            $table->timestamps();
        });

        Schema::table('T_P_SPEC05', function ($table) {
